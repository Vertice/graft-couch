// Source : developmentseed/backbone-couch
//
...
var Couch = module.exports = function(config) {
    this.config = url.parse('http://localhost:5984');
    _.extend(this.config, config);
    if (config.pathname == '/') throw 'Database name is required';

    this.uri = url.format(this.config);
    this.name = this.config.pathname.replace('/', '');
    _.bindAll(this, 'parse', 'put', 'post', 'del', 'get',
        'head', 'view', 'dbPut', 'dbDel', 'putDesignDocs');
};
...
// --------------------------
Couch.prototype.dbExists = function(callback) {
    request.get({
        uri: this.uri
    }, this.parse(callback));
};
